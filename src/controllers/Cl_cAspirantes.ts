...
export default class Cl_cAspirantes {
    ...
    async btRecargarOnClick(){
        ...
        const txtPorcentajeAprobados = document.getElementById("txtPorcentajeAprobados");
        if (txtPorcentajeAprobados) {
            const porcentaje = this.modelo.porcentajeAprobados();
            txtPorcentajeAprobados.innerText = `${porcentaje.toFixed(2)}%`;
        }
        const txtCalificacionMayor = document.getElementById("txtCalificacionMayor");
        if (txtCalificacionMayor) {
            const calificacion = this.modelo.calificacionMayor();
            txtCalificacionMayor.innerText = `${calificacion.toFixed(2)}`;
        }
        const txtCalificacionMenor = document.getElementById("txtCalificacionMenor");
        if (txtCalificacionMenor) {
            const calificacion = this.modelo.calificacionMenor();
            txtCalificacionMenor.innerText = `${calificacion.toFixed(2)}`;
        }
    }
}